...
export default function ExcelImporter({ onSuccess }: ExcelImporterProps) {
  ...
  useEffect(() => {
    ...
    announceMessage('Importador de datos desde Excel cargado', 'polite');
    if (settings.readAloud) {
      speakText('Importador de datos desde Excel. Seleccione el tipo de datos a importar');
    }
  }, []);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      announceMessage(`Archivo seleccionado: ${selectedFile.name}`, 'polite');
      if (settings.readAloud) {
        speakText(`Archivo ${selectedFile.name} seleccionado`);
      }
    }
  };

  const downloadTemplate = () => {
    announceMessage('Generando plantilla Excel', 'polite');
    if (settings.readAloud) {
      speakText('Descargando plantilla de Excel');
    }
    ...
    announceMessage(`Plantilla ${filename} descargada`, 'assertive');
    if (settings.readAloud) {
      speakText(`Plantilla descargada con el nombre ${filename}`);
    }
  };
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    setResult(null);
    announceMessage('Iniciando importación de datos', 'polite');
    if (settings.readAloud) {
      speakText('Procesando archivo, por favor espere');
    }
    ...
    try {
      ...
      if (data.length === 0) {
        const errorMsg = 'El archivo está vacío o no tiene el formato correcto';
        setResult({
          success: 0,
      ...
      const successMsg = `Importación completada: ${importResult.success} registros exitosos, ${importResult.errors.length} errores, ${importResult.warnings.length} advertencias`;
      announceMessage(successMsg, 'assertive');
      
      if (settings.readAloud) {
        speakText(`Importación completada. ${importResult.success} registros procesados exitosamente. ${importResult.errors.length} errores. ${importResult.warnings.length} advertencias`);
      }
      ...
      announceMessage(errorMsg, 'assertive');
      if (settings.readAloud) {
        speakText('Error al procesar el archivo');
      }
    } finally {
      setImporting(false);
    }
  ...
  const handleTypeChange = (type: 'students' | 'grades' | 'risk_factors') => {
    setImportType(type);
    setFile(null);
    setResult(null);
    ...
    announceMessage(`Tipo de importación cambiado a ${typeNames[type]}`, 'polite');
    if (settings.readAloud) {
      speakText(`Seleccionado: ${typeNames[type]}`);
    }
  };

  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-4xl mx-auto" role="main" aria-label="Importador de datos desde Excel">
      {/* Screen Reader Only Instructions */}
      <div className="sr-only" role="complementary" aria-label="Instrucciones de uso">
        <h2>Instrucciones para el importador:</h2>
        <ul>
          <li>Seleccione el tipo de datos a importar</li>
          <li>Descargue la plantilla correspondiente</li>
          <li>Complete los datos en el archivo Excel</li>
          <li>Seleccione y cargue el archivo completado</li>
          <li>Revise los resultados después de la importación</li>
        </ul>
      </div>

      <div 
        className="flex items-center gap-3 mb-6"
        onMouseEnter={() => settings.readAloud && speakText('Importar Datos desde Excel - Carga masiva de información académica')}
      >
      ...
      {/* Selector de tipo de importación */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2" id="import-type-label">
          Tipo de Datos a Importar
        </label>
        <div 
        ...
        >
          <button
            ...
            onMouseEnter={() => settings.readAloud && speakText('Importar estudiantes - Datos personales y carrera')}
            onFocus={() => announceMessage('Opción Estudiantes enfocada')}
            className={`p-4 rounded-lg border-2 transition-all focus:outline-none focus:ring-4 ${
              importType === 'students'
                ? 'border-blue-500 bg-blue-50 focus:ring-blue-300'
                : 'border-gray-200 hover:border-gray-300 focus:ring-gray-300'
            }`}
            role="radio"
            aria-checked={importType === 'students'}
            aria-label="Importar estudiantes: Datos personales y carrera"
          >
            <div className="font-semibold text-gray-800">Estudiantes</div>
            <div className="text-xs text-gray-600 mt-1">
              Datos personales y carrera
      ...
      <div 
        ...
        tabIndex={0}
        onMouseEnter={() => settings.readAloud && speakText('Instrucciones para importar datos')}
      >
        <div className="flex items-start gap-2">
      ...
      <div className="mb-6">
        <button
          onClick={downloadTemplate}
          onMouseEnter={() => settings.readAloud && speakText('Descargar plantilla de Excel')}
          onFocus={() => announceMessage('Botón Descargar Plantilla enfocado')}
          className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors focus:outline-none focus:ring-4 focus:ring-green-300"
          aria-label="Descargar plantilla de Excel para importar datos"
        >
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3">
          <label 
            htmlFor="file-input"
            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition-all cursor-pointer focus-within:ring-4 focus-within:ring-blue-300"
            onMouseEnter={() => settings.readAloud && speakText(file ? `Archivo seleccionado: ${file.name}` : 'Seleccionar archivo Excel')}
          >
            ...
            <input
              ...
              accept=".xlsx,.xls"
              onChange={handleFileChange}
              onFocus={() => announceMessage('Selector de archivo enfocado')}
              className="hidden"
          ...
          {file && (
            <button
              onClick={() => {
                setFile(null);
                announceMessage('Archivo removido', 'polite');
                if (settings.readAloud) {
                  speakText('Archivo removido');
                }
              }}
              onMouseEnter={() => settings.readAloud && speakText('Cancelar archivo seleccionado')}
              onFocus={() => announceMessage('Botón Cancelar enfocado')}
              className="px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors focus:outline-none focus:ring-4 focus:ring-gray-300"
              aria-label="Cancelar y remover archivo seleccionado"
            >
              Cancelar
            </button>
          )}
      ...
      <button
        onClick={handleImport}
        disabled={!file || importing}
        onMouseEnter={() => settings.readAloud && !importing && file && speakText('Importar datos desde archivo Excel')}
        onFocus={() => announceMessage('Botón Importar Datos enfocado')}
        className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg transition-colors focus:outline-none focus:ring-4 ${
          !file || importing
            ? 'bg-gray-300 text-gray-500 cursor-not-allowed focus:ring-gray-300'
            : 'bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-300'
        }`}
        aria-label={importing ? 'Importando datos, por favor espere' : !file ? 'Seleccione un archivo para importar' : 'Importar datos desde archivo Excel'}
        aria-disabled={!file || importing}
      >
        {importing ? (
          <>
      ...
      {result && (
        <div 
          ...
          role="region"
          aria-label="Resultados de la importación"
          aria-live="polite"
        >
          {/* Resumen */}
          <div className="grid grid-cols-3 gap-4">
            <div 
              ...
              tabIndex={0}
              role="status"
              aria-label={`${result.success} registros exitosos`}
              onMouseEnter={() => settings.readAloud && speakText(`${result.success} registros exitosos`)}
            >
              <div className="flex items-center gap-2 mb-1">
            ...
            <div 
              ...
              tabIndex={0}
              role="status"
              aria-label={`${result.warnings.length} advertencias`}
              onMouseEnter={() => settings.readAloud && speakText(`${result.warnings.length} advertencias`)}
            >
              <div className="flex items-center gap-2 mb-1">
            ...
            <div 
              ...
              tabIndex={0}
              role="status"
              aria-label={`${result.errors.length} errores`}
              onMouseEnter={() => settings.readAloud && speakText(`${result.errors.length} errores`)}
            >
              <div className="flex items-center gap-2 mb-1">
          ...
          {result.warnings.length > 0 && (
            <div 
              className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg"
              role="region"
              aria-label="Lista de advertencias"
            >
              <h3 className="font-semibold text-yellow-900 mb-2 flex items-center gap-2">
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto" role="list">
                {result.warnings.map((warning, index) => (
                  <div 
                    ...
                    role="listitem"
                    tabIndex={0}
                    onMouseEnter={() => settings.readAloud && speakText(`Fila ${warning.row}: ${warning.warning}`)}
                  >
                    <span className="font-medium">Fila {warning.row}:</span> {warning.warning}
                  </div>
                ))}
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto" role="list">
                {result.errors.map((error, index) => (
                  <div 
                    ...
                    role="listitem"
                    tabIndex={0}
                    onMouseEnter={() => settings.readAloud && speakText(`${error.row > 0 ? `Fila ${error.row}` : 'Error general'}: ${error.error}`)}
                  >
                    <span className="font-medium">
                      {error.row > 0 ? `Fila ${error.row}:` : 'Error general:'}
                    </span>{' '}
      ...
      <div 
        ...
        tabIndex={0}
        onMouseEnter={() => settings.readAloud && speakText('Notas importantes sobre la importación')}
      >
        <h4 className="font-semibold text-gray-800 mb-2">📝 Notas Importantes:</h4>
