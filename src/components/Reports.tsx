...
export default function Reports() {
  ...
  const loadReportData = async (isRefresh = false) => {
    if (isRefresh) {
      setRefreshing(true);
      announceMessage('Actualizando datos del reporte', 'polite');
    } else {
      setLoading(true);
      announceMessage('Cargando datos del reporte', 'polite');
    }
    try {
      const { data: students } = await supabase.from('estudiantes').select('id, nombre, apellido_paterno, apellido_materno');
      const { data: records } = await supabase.from('registros_estudiante_materia').select(`estado, semestre, calificacion_final, estudiantes!inner (id, nombre, apellido_paterno, carreras (nombre))`);
      ...
      const successMsg = isRefresh ? 'Reporte actualizado exitosamente' : 'Reporte cargado exitosamente';
      announceMessage(successMsg, 'assertive');
      
      if (settings.readAloud) {
        speakText(`${successMsg}. Promedio general: ${averageGrade.toFixed(1)}. Tasa de aprobación: ${approvalRate.toFixed(1)} por ciento`);
      }
    } catch (error) {
      console.error('Error loading report data:', error);
      const errorMsg = 'Error al cargar los datos del reporte';
...
const exportReport = () => {
  ...
  announceMessage('Generando reporte PDF', 'polite');
  if (settings.readAloud) {
    speakText('Generando reporte PDF, por favor espere');
  }
  ...
  // Descargar PDF
  const fileName = `reporte-calidad-${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);

  announceMessage('Reporte PDF generado y descargado exitosamente', 'assertive');
  if (settings.readAloud) {
    speakText(`Reporte PDF descargado con el nombre ${fileName}`);
  }
};
  ...
  return (
    <div className="space-y-6" role="main" aria-label="Reporte de análisis de calidad académica">
      {/* Screen Reader Only Instructions */}
      <div className="sr-only" role="complementary" aria-label="Instrucciones de navegación">
        <h2>Instrucciones de navegación del reporte:</h2>
        <ul>
          <li>Use Tab para navegar entre secciones y botones</li>
          <li>Presione Enter o Espacio para activar botones</li>
          <li>Los datos se anunciarán al enfocar elementos interactivos</li>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <div 
            ...
            onMouseEnter={() => settings.readAloud && speakText('Reporte Automático - Análisis integral de calidad académica')}
          >
          ...
          <div className="flex gap-2">
            <button 
              ...
              onMouseEnter={() => settings.readAloud && speakText('Botón Exportar Reporte')}
              onFocus={() => announceMessage('Botón Exportar Reporte enfocado')}
            ...
            <button 
              ...
              disabled={refreshing}
              onMouseEnter={() => settings.readAloud && speakText('Botón Reiniciar Vista')}
              onFocus={() => announceMessage('Botón Reiniciar Vista enfocado')}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-400 disabled:cursor-not-allowed focus:outline-none focus:ring-4 focus:ring-blue-300"
              aria-label={refreshing ? 'Actualizando datos, por favor espere' : 'Actualizar datos del reporte'}
            >
        ...
        >
          <div 
            ...
            onMouseEnter={() => settings.readAloud && speakText(`Promedio General: ${data.averageGrade.toFixed(1)}`)}
            tabIndex={0}
            role="article"
            aria-label={`Promedio general: ${data.averageGrade.toFixed(1)} basado en ${data.studentsWithGrades} calificaciones`}
          >
            <div className="flex items-center gap-2 mb-2">
              <Award className="w-5 h-5 text-blue-600" aria-hidden="true" />
              <span className="text-sm font-medium text-blue-900">Promedio General</span>
            </div>
            <p className={`text-3xl font-bold ${data.averageGrade >= 80 ? 'text-blue-700' : data.averageGrade >= 70 ? 'text-yellow-700' : 'text-red-700'}`}>
          ...
          <div 
            ...
            onMouseEnter={() => settings.readAloud && speakText(`Tasa de Aprobación: ${data.approvalRate.toFixed(1)} por ciento`)}
            tabIndex={0}
            role="article"
            aria-label={`Tasa de aprobación: ${data.approvalRate.toFixed(1)}% de ${data.totalRecords} registros`}
          >
            <div className="flex items-center gap-2 mb-2">
          ...
          <div 
            ...
            onMouseEnter={() => settings.readAloud && speakText(`Tasa de Reprobación: ${data.failureRate.toFixed(1)} por ciento`)}
            tabIndex={0}
            role="article"
            aria-label={`Tasa de reprobación: ${data.failureRate.toFixed(1)}%`}
          >
            <div className="flex items-center gap-2 mb-2">
              <TrendingDown className="w-4 h-4 text-yellow-600" aria-hidden="true" />
          ...
          <div 
            ...
            onMouseEnter={() => settings.readAloud && speakText(`Tasa de Deserción: ${data.dropoutRate.toFixed(1)} por ciento`)}
            tabIndex={0}
            role="article"
            aria-label={`Tasa de deserción: ${data.dropoutRate.toFixed(1)}%`}
          >
            <div className="flex items-center gap-2 mb-2">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div 
              ...
              onMouseEnter={() => settings.readAloud && speakText(`Excelente: ${data.gradeDistribution.excellent} estudiantes, ${((data.gradeDistribution.excellent/data.studentsWithGrades)*100).toFixed(1)} por ciento`)}
              tabIndex={0}
              aria-label={`Calificaciones excelentes (90 a 100): ${data.gradeDistribution.excellent} estudiantes, ${((data.gradeDistribution.excellent/data.studentsWithGrades)*100).toFixed(1)}%`}
            >
              <div className="text-xs text-gray-600 mb-1">Excelente (90-100)</div>
              <div className="text-2xl font-bold text-green-700">{data.gradeDistribution.excellent}</div>
              <div className="text-xs text-gray-500">{((data.gradeDistribution.excellent/data.studentsWithGrades)*100).toFixed(1)}%</div>
            ...
            <div 
              ...
              onMouseEnter={() => settings.readAloud && speakText(`Bueno: ${data.gradeDistribution.good} estudiantes, ${((data.gradeDistribution.good/data.studentsWithGrades)*100).toFixed(1)} por ciento`)}
              tabIndex={0}
              aria-label={`Calificaciones buenas (80 a 89): ${data.gradeDistribution.good} estudiantes, ${((data.gradeDistribution.good/data.studentsWithGrades)*100).toFixed(1)}%`}
            >
              <div className="text-xs text-gray-600 mb-1">Bueno (80-89)</div>
              <div className="text-2xl font-bold text-blue-700">{data.gradeDistribution.good}</div>
              <div className="text-xs text-gray-500">{((data.gradeDistribution.good/data.studentsWithGrades)*100).toFixed(1)}%</div>
            ...
            <div 
              ...
              onMouseEnter={() => settings.readAloud && speakText(`Satisfactorio: ${data.gradeDistribution.satisfactory} estudiantes, ${((data.gradeDistribution.satisfactory/data.studentsWithGrades)*100).toFixed(1)} por ciento`)}
              tabIndex={0}
              aria-label={`Calificaciones satisfactorias (70 a 79): ${data.gradeDistribution.satisfactory} estudiantes, ${((data.gradeDistribution.satisfactory/data.studentsWithGrades)*100).toFixed(1)}%`}
            >
              <div className="text-xs text-gray-600 mb-1">Satisfactorio (70-79)</div>
              <div className="text-2xl font-bold text-yellow-700">{data.gradeDistribution.satisfactory}</div>
              <div className="text-xs text-gray-500">{((data.gradeDistribution.satisfactory/data.studentsWithGrades)*100).toFixed(1)}%</div>
            ...
            <div 
              ...
              onMouseEnter={() => settings.readAloud && speakText(`Reprobado: ${data.gradeDistribution.failed} estudiantes, ${((data.gradeDistribution.failed/data.studentsWithGrades)*100).toFixed(1)} por ciento`)}
              tabIndex={0}
              aria-label={`Calificaciones reprobatorias (0 a 69): ${data.gradeDistribution.failed} estudiantes, ${((data.gradeDistribution.failed/data.studentsWithGrades)*100).toFixed(1)}%`}
            >
              <div className="text-xs text-gray-600 mb-1">Reprobado (0-69)</div>
              <div className="text-2xl font-bold text-red-700">{data.gradeDistribution.failed}</div>
              <div className="text-xs text-gray-500">{((data.gradeDistribution.failed/data.studentsWithGrades)*100).toFixed(1)}%</div>
        ...
        <div className="space-y-6">
          <div 
            ...
            role="region"
            aria-label="Factores de riesgo e impacto académico"
          >
            <h3 className="font-semibold text-gray-800 mb-3">Factores de Riesgo e Impacto Académico</h3>
            {data.topRiskFactors.length > 0 ? (
              <div className="space-y-2">
                {data.topRiskFactors.map((factor, index) => (
                  <div 
                    ...
                    onMouseEnter={() => settings.readAloud && speakText(`Factor ${index + 1}: ${factor.factor}, ${factor.count} casos, calificación promedio ${factor.avgGrade.toFixed(1)}, aprobación ${factor.approvalRate.toFixed(0)} por ciento`)}
                    tabIndex={0}
                    role="article"
                    aria-label={`Factor de riesgo ${index + 1}: ${factor.factor}. ${factor.count} casos registrados. Calificación promedio: ${factor.avgGrade.toFixed(1)}. Tasa de aprobación: ${factor.approvalRate.toFixed(0)}%`}
                  >
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-white text-sm font-semibold text-gray-700 shadow-sm" aria-hidden="true">
                      {index + 1}
                    </span>
                    <div className="flex-1">
                      <div className="font-medium text-gray-800">{factor.factor}</div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-center">
                        <div className={`px-3 py-1 rounded-full text-sm font-semibold ${getGradeColor(factor.avgGrade)}`}>
                          {factor.avgGrade.toFixed(1)}
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Calif. promedio</div>
                      </div>
                      <div className="text-center">
                        <div className={`px-3 py-1 rounded-full text-sm font-semibold ${factor.approvalRate >= 70 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                          {factor.approvalRate.toFixed(0)}%
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Aprobación</div>
                      </div>
                    </div>
              ...
              <p className="text-gray-500 text-center py-4" role="status">No hay factores de riesgo registrados</p>
            )}
          </div>
          ...
          <div 
            className="border-t pt-4"
            role="region"
            aria-label="Análisis por semestre"
          >
            <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
              <BookOpen className="w-5 h-5" aria-hidden="true" />
            ...
            {data.bySemester.length > 0 ? (
              <div className="space-y-2">
                {data.bySemester.map((sem, index) => (
                  <div 
                    key={index} 
                    className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg"
                    onMouseEnter={() => settings.readAloud && speakText(`Semestre ${sem.semester}, ${sem.total} registros, promedio ${sem.avgGrade.toFixed(1)}, ${sem.failed} reprobados, ${sem.dropout} deserciones`)}
                    tabIndex={0}
                    role="article"
                    aria-label={`Semestre ${sem.semester}: ${sem.total} registros. Promedio: ${sem.avgGrade.toFixed(1)}. ${sem.failed} reprobados y ${sem.dropout} deserciones`}
                  >
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-sm font-semibold text-blue-700" aria-hidden="true">
                      {sem.semester}
                    </span>
                    <div className="flex-1">
                      <div className="font-medium text-gray-800">Semestre {sem.semester}</div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-center">
                        <div className={`px-3 py-1 rounded-full text-sm font-semibold ${getGradeColor(sem.avgGrade)}`}>
                          {sem.avgGrade.toFixed(1)}
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Promedio</div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm">
                          <span className="text-red-700 font-semibold">{sem.failed}</span>
                          <span className="text-gray-500 mx-1">/</span>
                          <span className="text-orange-700 font-semibold">{sem.dropout}</span>
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Rep. / Des.</div>
                      </div>
                    </div>
              ...
              <p className="text-gray-500 text-center py-4" role="status">No hay datos por semestre</p>
            )}
          </div>
          ...
          <div 
            className="border-t pt-4"
            role="region"
            aria-label="Análisis por carrera"
          >
            <h3 className="font-semibold text-gray-800 mb-3">Análisis por Carrera</h3>
            {data.byMajor.length > 0 ? (
              <div className="space-y-2">
                {data.byMajor.map((major, index) => (
                  <div 
                    key={index} 
                    className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg"
                    onMouseEnter={() => settings.readAloud && speakText(`Carrera ${major.major}, ${major.total} registros, promedio ${major.avgGrade.toFixed(1)}, ${major.failed} reprobados, ${major.dropout} deserciones`)}
                    tabIndex={0}
                    role="article"
                    aria-label={`Carrera: ${major.major}. ${major.total} registros. Promedio: ${major.avgGrade.toFixed(1)}. ${major.failed} reprobados y ${major.dropout} deserciones`}
                  >
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-white text-sm font-semibold text-gray-700 shadow-sm" aria-hidden="true">
                      {index + 1}
                    </span>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <div className="text-center">
                        <div className={`px-3 py-1 rounded-full text-sm font-semibold ${getGradeColor(major.avgGrade)}`}>
                          {major.avgGrade.toFixed(1)}
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Promedio</div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm">
                          <span className="text-red-700 font-semibold">{major.failed}</span>
                          <span className="text-gray-500 mx-1">/</span>
                          <span className="text-orange-700 font-semibold">{major.dropout}</span>
                        </div>
                        <div className="text-xs text-gray-500 mt-1">Rep. / Des.</div>
                      </div>
                    </div>
              ...
              <p className="text-gray-500 text-center py-4" role="status">No hay datos por carrera</p>
            )}
          </div>
          ...
          <div 
            className="border-t pt-4"
            role="region"
            aria-label="Estudiantes en situación crítica"
          >
            <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
            ...
            {data.criticalStudents.length > 0 ? (
              <div className="space-y-2">
                {data.criticalStudents.map((student, index) => (
                  <div 
                    key={index} 
                    className="flex items-center gap-3 bg-red-50 border border-red-200 px-4 py-3 rounded-lg"
                    onMouseEnter={() => settings.readAloud && speakText(`Estudiante en riesgo: ${student.name}, calificación ${student.grade.toFixed(1)}, ${student.riskFactors} factores de riesgo`)}
                    tabIndex={0}
                    role="article"
                    aria-label={`Estudiante en situación crítica ${index + 1}: ${student.name}. Calificación: ${student.grade.toFixed(1)}. ${student.riskFactors} factores de riesgo`}
                  >
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-red-200 text-sm font-semibold text-red-700" aria-hidden="true">
                      {index + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-gray-800">{student.name}</div>
              ...
              <div 
                className="text-center py-8 bg-green-50 border border-green-200 rounded-lg"
                role="status"
                aria-label="No hay estudiantes en situación crítica"
              >
                <Award className="w-12 h-12 text-green-500 mx-auto mb-2" aria-hidden="true" />
                <p className="text-green-700 font-medium">No se identificaron estudiantes en situación crítica</p>
                <p className="text-sm text-green-600 mt-1">Todos los estudiantes con factores de riesgo mantienen calificaciones aprobatorias</p>
              </div>
            )}
